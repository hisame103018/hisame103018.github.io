...
const UPLOADS_FOLDER = path.join('C:\\EJLee\\Util\\nginx-1.24.0\\html\\uploads', 'uploads');
...
// 삭제 처리
app.get('/md_deletePost/:id', async (req, res) => {
    // 로그인 여부 확인
    if (!req.session.loggedIn) {
        return res.redirect('/login'); // 로그인되지 않은 경우 로그인 페이지로 리다이렉트
    }
...
// 댓글 삭제
app.post('/md_deleteComment/:id', async (req, res) => {
    // 로그인 여부 확인
    if (!req.session.loggedIn) {
...
// 댓글 수정
app.post('/md_editComment/:comment_id', async (req, res) => {
